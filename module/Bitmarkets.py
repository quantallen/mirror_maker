...
class Spot:
    version = 'v3.2'
    testnet_endpoint = f'https://testapi.bitmarkets.io/spot/api/{version}'
    production_endpoint = f'https://api.bitmarkets.com/spot/api/{version}'
    staging_endpoint = f'https://bitmarkets-api.btse.co/spot/api/{version}'
    ...
    async def amend_order(self, symbol, type, value=None, price=None, size=None, order_id=None, cl_order_id=None):
        """
        v2.1
        symbol: e.g. 'BTCPFC'
        type: PRICE|SIZE
        ...
        #print("amend order :",resp)
        return resp

    async def amend_order_size(self, symbol, cl_order_id, size):
        """
        Example:
            symbol='BTSE-USD',
        v3: Returns a list
        """
        order_form = {'symbol': symbol,
                      'clOrderID': cl_order_id,
                      'type': 'SIZE',
                      'value':  f'{size:.5f}'}
        resp = await self.put('/order', order_form)
        
        return resp

    async def amend_order_price(self, symbol, cl_order_id, price):
        """
        Example:
            symbol='BTSE-USD',
        v3: Returns a list
    ...
    async def cancel_order(self, symbol, cl_order_id):
        try:
            resp = await self.delete('/order', params={'symbol': symbol, 'clOrderID': cl_order_id})
            resp = resp[0]
        except:
            pass
        try:
            logger_string = f"Cancel {symbol} {self.order_cache.pop(cl_order_id)['description']}"
        except KeyError:
            logger_string = f'Cancel {symbol} order {cl_order_id}'
        try:
    ...
    async def cancel_all_orders(self, symbol):
        ...
        resp = await self.delete('/order', params=params)
        return resp
    ...
    async def get_open_orders(self, symbol, order_id=None):
        """
        Example:
            symbol='BTSE-USD',
        v3: Returns a list
...
class Future:
    version = 'v2.1'
    testnet_endpoint = f'https://testapi.bitmarkets.io/futures/api/{version}'
    production_endpoint = f'https://api.bitmarkets.com/futures/api/{version}'
    staging_endpoint = f'https://bitmarkets-api.btse.co/futures/api/{version}'
    ...
    def __init__(self, key=None, secret=None, mode='production'):
        ...
        self.logger = logging.getLogger('Bitmarkets.Future')
        #self.logger.setLevel('INFO')
        # logging.basicConfig(
    ...
    def _make_headers(self, path, data=''):
        nonce = str(int(time.time()*1000))
        #print(data)
        #data = json.dumps(data, cls = JSONEncoder) if data else ''
    ...
    async def get_position(self, symbol):
        order_form = {'symbol': symbol}
        resp = await self.get('/user/positions', params=order_form)
        return resp
    async def get_margin(self, symbol):
        order_form = {'symbol': symbol,'count':1}
        resp = await self.get('/user/margin', params=order_form)
        return resp
