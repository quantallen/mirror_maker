...
class Future:
    ...
    async def _request(self, method, path, params={}, data=None):
        ...
        if method in ['GET', 'DELETE']:
            ...
            resp = await async_request(method, self.endpoint + path, headers=headers, data=json.dumps(data,cls=JSONEncoder))
            #print("resp",resp)
        try:
            return json.loads(resp)
        except json.decoder.JSONDecodeError:
    ...
    async def get_trades(self, symbol, params={}):
        ...
        resp = await self.get_public('/trades', params=params1)
        return resp
    async def get_trades_history(self, symbol,startTime = None,endTime = None,clOrderID = None, params={}):
        """
        Example:
            symbol='BTSE-USD',
            params={'count': 500}
        v3: Returns a list
        """
        params1 = {'symbol': symbol,'startTime':startTime,'endTime':endTime,'count': 1}
        params1.update(params)
        resp = await self.get('/user/trade_history', params=params1)
        #print(resp)
        return resp
